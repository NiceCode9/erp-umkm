...
<div class="max-w-screen-xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-foreground">Piutang</h1>
            <p class="text-muted-foreground">Transaksi cabang ini yang belum lunas</p>
        </div>
    </div>
            ...
            @forelse($sales as $sale)
                <tr class="hover:bg-muted/50 transition">
                    ...
                    <td class="px-4 py-3 text-center">
                        <x-badge variant="{{ $sale->payment_status === 'paid' ? 'success' : ($sale->payment_status === 'partial' ? 'warning' : 'danger') }}">{{ $sale->payment_status }}</x-badge>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <a href="{{ route('app.kasir.receivables.pay', $sale) }}" class="text-secondary hover:text-secondary/80 text-sm">Bayar</a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="9" class="px-4 py-12 text-center text-muted-foreground">Semua transaksi sudah lunas</td></tr>
            @endforelse
        </tbody>
    </x-data-table>
</div>
